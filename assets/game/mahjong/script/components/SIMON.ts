...
@ccclass('SIMON')
@menu('Mahjong/SIMON')
export class SIMON extends Component implements BaseGame {
    /** 餘額 */
    ...
    protected runTransition(transitionType: TransitionType): Promise<void> {
        return new Promise(async (resolve) => {
            if (transitionType == TransitionType.MAIN && this.freeTopBg.active) {
                ...
                const freeGamePayoffTotal: number = this.free.FreeGamePayoffTotal + this.payoff;
                if (freeGamePayoffTotal > 0) {
                    await this.totalWin.running(freeGamePayoffTotal);
                }
    ...
    private async winProcess() {
        ...
        if (this.cards.length - 1 == this.currentCardIndex && this.bonus.WinCard) { // 胡了！
            // 先處理 眼睛
            let arrWinSymbol: Array<MahjongSymbol> = (<MahjongRoller>this.roller).getPairSymbol(this.bonus["EyeCard"]);
            let len: number = arrWinSymbol.length;
            if (len == 2) { // 不是用算牌區(碰或槓)的元素作眼睛
                ...
                for (let i: number = 0; i < len; i++) {
                    if (i == len - 1) {
                        await arrWinSymbol[i].getComponent(MahjongSymbol).win(this.symbolWinLayer);
                    } else {
                        arrWinSymbol[i].getComponent(MahjongSymbol).win(this.symbolWinLayer);
                    }
                }
                await this.fadeOutWinLayer();

                // 將眼睛放置算牌區
                await this.calculationCupboard.getComponent(CalculationCupboard).place(WinLineType.PAIR, this.bonus.EyeCard, arrWinSymbol, this.roller.doSpeed);
            }
