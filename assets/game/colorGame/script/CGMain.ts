...
export class CGMain extends Component {
    ...
    //新局開始
    public newRound() {
        this.model.getRoundData();//獲取新局資料
        this.model.getBetData();//獲取下注資料
        // this.model.getTopUserInfo();//獲取前三名玩家資料
        // this.model.getRoundData();//獲取本回合資料
        // this.model.getBetInfo();//獲取下注資料
        this.controller.updateRoadMap();//更新路紙(每局更新)
    ...
    private async betStart() {
        ...
        }, 1, this.model.loadInfo.betTime - 1, 1)
    }
    ...
    public setBetTime(timer: number) {
        ...
        const betTotalTime = this.model.loadInfo.betTime;
    ...
    private async otherPlayerBet() {
        ...
        for (let i = 1; i < 5; i++) {
            if (Math.random() > 0.5) {
                ...
                const randomChipID = Math.floor(Math.random() * this.model.loadInfo.chipRange.length);
                this.gameChipControl.createChipToBetArea(randomBetArea, i, randomChipID, false);
                //判斷本地玩家是否跟注
                if (i < 4)
                    if (this.gameView.btnStopCall[i - 1].active) {
                        this.gameChipControl.createChipToBetArea(randomBetArea, 0, this.model.touchChipID, false);//玩家跟注
                    }
            }
        }
    }
    ...
    private async betStop() {
        ...
        this.gameChipControl.updateRebetData();//更新寫入續押資料
        this.model.getRewardInfo();//獲取開獎結果
        this.runDice();
    }
    ...
    private async runDice() {
        let winNumber = this.model.rewardInfo.winNumber;
        ...
        for (let i of winNum) {
            ...
            //判斷玩家是否有下注該中獎區
            if (this.model.loadInfo.betAreaCredit[i] > 0) {
                this.model.loadInfo.betAreaCredit[i] *= betCount[i] === 3 ? 10 : betCount[i] + 1;//注區分數變更(倍率)
                localWinScore += this.model.loadInfo.betAreaCredit[i];
                this.gameView.betInfo.children[i].getChildByName('BetScore').getComponent(Label).string = UtilsKitS.NumDigits(this.model.loadInfo.betAreaCredit[i]);
        ...
        for (let i of loseNum) {
            this.model.loadInfo.betAreaCredit[i] = 0;
            this.gameView.betInfo.children[i].getChildByName('BetScore').getComponent(Label).string = UtilsKitS.NumDigits(this.model.loadInfo.betAreaCredit[i]);
