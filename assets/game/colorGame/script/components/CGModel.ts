...
export class CGModel {
    ...
    public betAreaPercent: number[] = [0, 0, 0, 0, 0, 0];//各下注區總額百分比(前端計算)
    ...
    public setBetData(data: BetInfo) {
        this.betInfo = data;
        const total = this.betInfo.betAreaTotalCredit.reduce((a, b) => a + b, 0);
        this.betAreaPercent = this.betInfo.betAreaTotalCredit.map(credit => (total > 0 ? credit / total * 100 : 0));
    }
    ...
    //回傳接收下注成功後的本地用戶資料
    public setBeginGameInfo(data: onBeginGameInfo) {
