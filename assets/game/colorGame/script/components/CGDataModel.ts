...
import { onLoadInfo, RankingInfo, GameState, onJoinGame, PayoffInfo, BeginGameData } from '../connector/receive/CGReceive';
...
//模擬後端給的資料
@ccclass('CGDataModel')
export class CGDataModel {
    private static singleton: CGDataModel = null;
    public static getInstance(): CGDataModel {
        if (!CGDataModel.singleton) {
            CGDataModel.singleton = new CGDataModel();
        }
        return CGDataModel.singleton;
    }
