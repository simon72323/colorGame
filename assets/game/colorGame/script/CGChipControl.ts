...
export class CGChipControl extends Component {
    //Script
    @property({ type:  CGView, tooltip: "遊戲UI腳本" })
    private gameView: CGView = null;
    @property({ type: CGController, tooltip: "遊戲控制腳本" })
    ...
    onLoad() {
        ...
        tempPool = [];
        this.gameView.rebetScoreLable.string = '0';
        this.model = CGModel.getInstance();
    }

    //更新寫入續押資料
    public updateRebetData() {
        const betTotalScore = this.model.loadInfo.betTotalCredit;
        if (betTotalScore <= 0)
            return;
        this.rebetSave = [[], [], [], [], [], []];//清空後重新記錄籌碼
        for (let i = 0; i < this.rebetSave.length; i++) {
            for (let chip of this.gameView.chipDispatcher.getChildByName('MainPlayer').children[i].children) {
                this.rebetSave[i].push(chip.getComponent(SetChipID).chipID);//添加籌碼id到續押資料內
            }
        }
        this.rebetScore = betTotalScore;
        this.gameView.rebetScoreLable.string = UtilsKitS.NumDigits(this.rebetScore);
    }
    ...
    public setRebet(event: Event, state: string) {
        switch (state) {
            case 'init'://未續押狀態
                ...
                this.gameView.btnRebet.active = true;
                this.gameView.btnAuto.active = false;
                this.gameView.btnAutoStop.active = false;
                break;
            case 'onceBet'://單次續押狀態
                this.rebetState = state;
                if (this.gameView.btnRebet.active)
                    this.runRebet();
                else {
                    this.showTipMessage("停止自動續押", 0);
                    this.gameView.btnAuto.active = true;
                    this.gameView.btnAutoStop.active = false;
                }
                break;
            case 'autoBet'://自動續押狀態
                ...
                this.rebetState = state;
                this.gameView.btnRebet.active = false;
                this.gameView.btnAuto.active = false;
                this.gameView.btnAutoStop.active = true;
                break;
        }
    }
    ...
    public runRebet() {
        ...
        if (this.model.loadInfo.credit < this.rebetScore) {
            this.setRebet(null, 'init');
    ...
    public createChipToBetArea(betId: number, playerId: number, chipID: number, rebet: boolean) {
        ...
        let chipScore = this.model.loadInfo.chipRange[chipID];
        if (playerId === 0) {
            if (this.model.loadInfo.betTotalCredit + chipScore > this.model.loadInfo.limit) {
                this.showTipMessage("投注超過限額", 0);
                return;
            }
            if (this.model.loadInfo.credit < chipScore) {
                this.showTipMessage("餘額不足", 0);
                for (let i = 0; i < 3; i++) {
                    this.gameView.updateBetCallUI(i, false);//取消跟注
                }
                return;
            }
            this.gameView.btnRebet.getComponent(Button).interactable = false;
            betChip = this.gameView.chipDispatcher.getChildByName('MainPlayer').children[betId];//下注區節點
            ...
            console.log("籌碼分數",chipScore)
            poolBetChip.children[0].getComponent(Label).string = UtilsKitS.NumDigits(chipScore);//設置籌碼分數
            poolBetChip.parent = betChip;
            this.model.loadInfo.betTotalCredit += chipScore;//本地下注總分增加

            this.model.loadInfo.credit -= chipScore;
            //*****注意續押的籌碼編號可能會跟選擇區的籌碼不同，如果執行續押，籌碼會從玩家頭像下注 */
            if (rebet)
                poolBetChip.position = this.gameView.playerPos.children[0].getWorldPosition().subtract(betChip.worldPosition);
            else
                poolBetChip.position = this.gameView.touchChip.children[this.model.chipSetID.indexOf(chipID)].getWorldPosition().subtract(betChip.worldPosition);

            this.rebetScore = this.model.loadInfo.betTotalCredit;
            this.gameView.rebetScoreLable.string = UtilsKitS.NumDigits(this.rebetScore);//續押分數更新
            this.gameView.comBtnBet.getComponent(Animation).play();
        } else {
            betChip = this.gameView.chipDispatcher.getChildByName('OtherPlayer').children[betId];//下注區節點
            this.gameView.playerPos.children[playerId].getComponent(Animation).play();//頭像移動
            poolBetChip = this.pool.get(this.betChipBlack);
            poolBetChip.parent = betChip;
            poolBetChip.position = this.gameView.playerPos.children[playerId].getWorldPosition().subtract(betChip.worldPosition);
        }
        // this.model.betInfo.BetAreaData[betId].BetCredit += chipScore;//注區分數增加
        this.model.betInfo.betAreaTotalCredit[betId] += chipScore;//各注區總分更新
        ...
        tween(poolBetChip).to(0.3, { position: movePos }, { easing: 'sineOut' })
            .call(() => {
                if (playerId === 0) {
                    this.model.loadInfo.betAreaCredit[betId] += chipScore;//本地玩家該下注區總分數
                    this.gameView.betInfo.children[betId].getChildByName('BetScore').getComponent(Label).string = UtilsKitS.NumDigits(this.model.loadInfo.betAreaCredit[betId]);
                }
                else {
                    poolBetChip.children[0].getComponent(Label).string = UtilsKitS.NumDigits(chipScore);
                }
                this.controller.updateUIScore(); //更新介面分數
                this.chipScale(poolBetChip);//籌碼縮放動態
            }).start();
    }

    //更新注區資訊(下注區,下注人,下注分數)
    // public updateBetInfo(betId: number, playerId: number, score: number) {
    //     if (playerId === 0) {
    //         this.model.loadInfo.BetAreaCredit[betId] += score;//本地該下注區總分增加
    //         this.model.loadInfo.Credit -= score;
    //         this.gameView.comBtnBet.getComponent(Animation).play();
    //     }
    //     this.model.betInfo.BetAreaData[betId].BetCredit += score;//注區總分更新
    //     this.gameView.updateUIScore(); //更新介面分數
    // }
    ...
    public recycleChip(betId: number) {
        //回收籌碼到莊家位置並清除
        for (let i = 0; i < 2; i++) {
            const betChip = this.gameView.chipDispatcher.children[i].children[betId];//下注區節點
            const savePos = betChip.getPosition();
            const movePos = this.gameView.chipDispatcher.getChildByName('BankerPos').getWorldPosition().subtract(betChip.getWorldPosition()).add(savePos);
    ...
    public createPayChipToBetArea(betId: number, multiply: number) {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            if (betChip.children.length > 0) {
                ...
                payNode.position = this.gameView.chipDispatcher.getChildByName('BankerPos').getWorldPosition().subtract(betChip.getWorldPosition());
    ...
    private chipPayToPlayer(betId: number) {
        for (let i = 0; i < 2; i++) {
            const betChip = this.gameView.chipDispatcher.children[i].children[betId];
            for (let j = 0; j < betChip.children.length; j++) {
                ...
                const movePos = this.gameView.playerPos.children[chipPlayerID].getWorldPosition().subtract(betChip.getWorldPosition());
    ...
    public async showTipMessage(tx: string, colorID: number) {
        ...
        tip.parent = this.gameView.tipMessage;
        switch (colorID) {
            case 0:
